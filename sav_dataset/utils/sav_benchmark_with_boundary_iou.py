...
class VideoEvaluator:
    ...
    def __call__(self, vid_name: str) -> Tuple[str, Dict[str, float], Dict[str, float]]:
        ...
        for all_frames, obj_id, gt_path, pred_path in to_evaluate:
            ...
            eval_results.append((obj_id, iou, boundary_f, boundary_iou))

        if is_sav_format:
            iou_output, boundary_f_output, boundary_iou_output = self.consolidate(eval_results)
        else:
            assert len(eval_results) == 1
            iou_output = eval_results[0][1]
            boundary_f_output = eval_results[0][2]
            boundary_iou_output = eval_results[0][3]

        return vid_name, iou_output, boundary_f_output, boundary_iou_output
    ...
    def consolidate(
        ...
        boundary_f_output = {}
        boundary_iou_output = {}
        for obj_id, iou, boundary_f, boundary_iou in eval_results:
            assert len(iou) == 1
            key = list(iou.keys())[0]
            iou_output[obj_id] = iou[key]
            boundary_f_output[obj_id] = boundary_f[key]
            boundary_iou_output[obj_id] = boundary_iou[key]
        return iou_output, boundary_f_output, boundary_iou_output
...
def benchmark(
    ...
    *,
    ...
    all_global_jf, all_global_j, all_global_f, all_global_biou = [], [], [], []
    all_object_metrics = []
    for i, mask_root in enumerate(mask_roots):
        ...
        all_boundary_iou = []
        object_metrics = {}
        for name, iou, boundary_f, boundary_iou in results:
            all_iou.extend(list(iou.values()))
            all_boundary_f.extend(list(boundary_f.values()))
            all_boundary_iou.extend(list(boundary_iou.values()))
            object_metrics[name] = (iou, boundary_f, boundary_iou)

        global_j = np.array(all_iou).mean()
        global_f = np.array(all_boundary_f).mean()
        global_jf = (global_j + global_f) / 2
        global_biou = np.array(all_boundary_iou).mean()
        ...
        # build header
        out_string = f'{"sequence":<{ml}},{"obj":>3}, {"J&F":>4}, {"J":>4}, {"F":>4}, {"B_IoU":>5}\n'
        out_string += f'{"Global score":<{ml}},{"":>3}, {global_jf:.1f}, {global_j:.1f}, {global_f:.1f}, {global_biou:.1f}\n'
        # append one line for each object
        for name, (iou, boundary_f, boundary_iou) in object_metrics.items():
            for object_idx in iou.keys():
                j, f, biou = iou[object_idx], boundary_f[object_idx], boundary_iou[object_idx]
                jf = (j + f) / 2
                out_string += (
                    f"{name:<{ml}},{object_idx:03}, {jf:>4.1f}, {j:>4.1f}, {f:>4.1f}, {biou:>5.1f}\n"
                )
        ...
        if verbose:
            ...
            print("\nSummary:")
            print(
                f"Global score: J&F: {global_jf:.1f} J: {global_j:.1f} F: {global_f:.1f} B_IoU: {global_biou:.1f}"
            )
        ...
        all_global_biou.append(global_biou)
        all_object_metrics.append(object_metrics)

    return all_global_jf, all_global_j, all_global_f, all_global_biou, all_object_metrics
