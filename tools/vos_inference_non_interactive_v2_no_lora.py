...
# ---------------------------------------------------------------------------
# Fusion (dual-path) pipeline: forward video + per-frame image, no backward.
# Matches the training scheme in forward_tracking_with_fusion:
#   w_v     = max(fwd_iou),  w_i = max(img_iou)
#   weights = softmax([w_v, w_i])
#   fused   = weights[0] * fwd_logit + weights[1] * img_logit
# ---------------------------------------------------------------------------
...
def vos_non_interactive_fusion_inference(
    ...
    """Dual-path fusion inference aligned to training.

    Path 1 – forward video tracking: first-frame image seg → propagate forward.
    Path 2 – per-frame image seg: each frame independently (no temporal context).
    Fusion: IoU-weighted softmax, identical to forward_tracking_with_fusion.
    """
    video_dir = os.path.join(base_video_dir, video_name)
    frame_names = sorted(
        [
    ...
    for out_frame_idx, out_obj_ids, out_mask_logits, out_ious in (
        ...
        fwd_results[out_frame_idx] = (
            ...
            out_ious[0].cpu().float(),     # [1] or [M]
        )
    fwd_results[0] = (fwd_raw_logit_0, fwd_iou_0)

    # ── Path 2: per-frame image-only segmentation ─────────────────────────────
    predictor.reset_state(inference_state)
    img_results = {}  # frame_idx → (logit [1,1,H,W], iou tensor)
    ...
    # ── IoU-weighted softmax fusion (matches training) ────────────────────────
    os.makedirs(os.path.join(output_mask_dir, video_name), exist_ok=True)
    ...
    for i in range(num_frames):
        ...
        # Take max over mask candidates → scalar per path, then softmax.
        # Identical to training: w_v = vid_iou.max(), w_i = img_iou.max()
        w_v = fwd_iou.float().max().unsqueeze(0).to(device)  # [1]
        w_i = img_iou.float().max().unsqueeze(0).to(device)  # [1]
        weights = torch.softmax(torch.cat([w_v, w_i]), dim=0)  # [2]

        fused = (
            weights[0] * fwd_logit.float()
            + weights[1] * img_logit.float()
        )  # [1,1,H,W]

        per_obj_output_mask = {0: (fused.squeeze() > score_thresh).cpu().numpy()}
        save_masks_to_dir(
            output_mask_dir=output_mask_dir,
            video_name=video_name,
...
def main():
    parser = argparse.ArgumentParser(
        description="Non-interactive VOS inference (standard / dual fusion)"
    )
    ...
    parser.add_argument("--use_fusion", action="store_true",
                        help="Dual-path fusion: forward video + image (matches training)")
    ...
    mode = "dual fusion (forward + image)" if args.use_fusion else "standard"
    print(
