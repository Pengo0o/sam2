...
def segment_frame_non_interactive(
    ...
    Returns:
        binary_mask : bool numpy array [H, W]  — for add_new_mask / foreground check
        iou         : float32 tensor [1]        — real IoU from SAM decoder
        raw_logit   : float32 tensor [1,1,H,W]  — soft logit at video resolution
                      (Bug 1 fix: used directly in fusion instead of ±10 conversion)
    """
...
# ---------------------------------------------------------------------------
# Fusion (three-path) pipeline  — all three bugs fixed
# ---------------------------------------------------------------------------
...
def vos_non_interactive_fusion_inference(
    ...
    for scan_idx in range(num_frames - 1, -1, -1):
        ...
        if mask_scan.any():
            ...
            if iou_scan.item() >= bwd_iou_thresh:
                ...
                break  # found a high-quality anchor
    ...
    predictor.reset_state(inference_state)
    # img_results stores the RAW LOGIT, not a binary mask (Bug 1 fix).
...
def main():
    parser = argparse.ArgumentParser(
        description="Non-interactive VOS inference v2 (three-bug fix)"
    )
    ...
    mode = "fusion (v2)" if args.use_fusion else "standard"
    print(
